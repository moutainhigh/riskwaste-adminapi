...
public enum AssociatedUserTypeDic implements EnumBase {
    /** 管理员 **/
    ADMIN(1),
    /** 业务员 **/
    DB(2),
     ...
     */
    @JsonCreator
    public static AssociatedUserTypeDic getItem(final int code) {
        final EnumBase result = valueMap.get(code);
        if (result == null) {
            throw new IllegalArgumentException("输入的code" + code + "不在枚举的取值范围内");
        }
        return (AssociatedUserTypeDic) result;
    }
     ...
     * 构造器，传入code
     */
    AssociatedUserTypeDic(final int code) {
        this.code = code;
    }
